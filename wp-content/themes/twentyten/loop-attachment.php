...
$(document).ready(function() {
	$('.attachment-900x900').click(function(){
		var img = $('.attachment-900x900').attr('src');
		$("#ajaxloader").show();
		$.post("http://localhost/covertimeline/fb/fbmain.php", {img:img },
			function(data) {
				alert(img);
			});
...
?>
						<p class="attachment"><a  class='covertimeline' href="#" title="<?php echo esc_attr( get_the_title() ); ?>" rel="attachment"><?php
							$attachment_width  = apply_filters( 'twentyten_attachment_size', 900 );
							$attachment_height = apply_filters( 'twentyten_attachment_height', 900 );
															
							echo wp_get_attachment_image( $post->ID, array( $attachment_width, $attachment_height ) ); // filterable image width with, essentially, no limit for image height.
						?></a></p>
						<div style="display:none;position: absolute;top: 60%;left: 48%; id="ajaxloader">
							<img src='http://mt0.bochstatic.com/honda/images/ajax-loader.gif'/>
						</div>
